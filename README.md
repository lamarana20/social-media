...
- Post likes toggle per user; comment likes per user
...
- Combined local loop (PHP server, queue, logs, Vite) is available via Composer script `composer dev` (uses `concurrently`)
...
- Email verification flow at `/email/verify`
- Password reset begins at `/forgot-password`
...
- `app/Models` — Post, Comment, Like, JaimePost, User
- `app/Http/Controllers` — Auth, Post, Comment, Dashboard, ResetPassword
